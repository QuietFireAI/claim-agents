...
1. **dispatcher-agents** (reference) - the vendored core in `dispatcher/`
   IS that runtime; nothing to mount, it ships here and the 17-test suite
   proves it.
...
- `dispatcher/claim_spokes.py` - reference handlers; port or wrap,
...
5. Run the acceptance gate: the host-mounted identity must pass the same
   assertions as `tests_claim/` - closed track rejects an illegal
