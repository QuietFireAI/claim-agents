...
class GovernedIdentity:
    ...
    def _wire_claim(self, m):
        """Wire the 14 P&C claim spokes exactly as the e2e harness does."""
        s = {
            "01": m.Spoke01FNOLIntake(self.hub),
            "02": m.Spoke02ClaimTriage(self.hub),
            "03": m.Spoke03PolicyVerification(self.hub, policy_db={
                "POL-1": {"status": "in_force", "effective": "2026-01-01"}}),
            "04": m.Spoke04ClaimantComm(self.hub, templates={
                "ack": "We received your claim.",
                "status": "Update on your claim."}),
            "05": m.Spoke05DocumentCollection(self.hub),
            "06": m.Spoke06InspectionScheduling(self.hub),
            "07": m.Spoke07VendorCoordination(self.hub,
                                              panel={"inspection": "v-insp"}),
            "08": m.Spoke08EstimateValuation(self.hub),
            "09": m.Spoke09FraudSignals(self.hub),
            "10": m.Spoke10Subrogation(self.hub),
            "11": m.Spoke11PaymentsReserves(self.hub),
            "12": m.Spoke12ComplianceDeadlines(self.hub),
            "13": m.Spoke13ClaimFileRecords(self.hub),
            "14": m.Spoke14DailyOperations(self.hub),
        }
        for aid, sp in s.items():
            if aid != "01":
                self.hub.register(aid, sp.handle)
    ...
    def _drive(self, intent, payload, ctx):
        ...
        if modname.endswith("claim_spokes"):
            if intent == "fnol.capture":
                # front door: first notice of loss (P01 entry)
                self.spokes["01"].capture_fnol(ctx, payload["fnol"])
                st = self.spokes["02"].claims.get(ctx, {})
                return {"ok": True, "context_id": ctx,
                        "assigned": st.get("assigned", False),
                        "represented": st.get("represented", False),
                        "note": "FNOL captured; the swarm runs triage, "
                                "policy facts, and inspection dispatch behind "
                                "the gates",
                        "audit_entries": len(self.hub.audit.read())}
            return {"ok": False, "context_id": ctx,
                    "reason": f"'{intent}' is not a legal front-door intent "
                              f"for claim; the entry lane is 'fnol.capture'"}
