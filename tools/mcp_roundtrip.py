...
async def main():
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            line("=" * 64)
            line("  LIVE MCP ROUND TRIP - claim identity over stdio")
            ...
            r = await call("identity.submit",
                           {"intent": "fnol.capture",
                            "payload": {"fnol": {"insured": "i1", "policy_number": "POL-1", "date_of_loss": "2026-07-01", "loss_description": "tree fell on garage roof"}}})
            line(f"  ok: {r.get('ok')}  assigned: {r.get('assigned')}  note: {r.get('note','')[:50]}")
            ...
            r = await call("identity.authority",
                           {"intent": "payment.authority",
                            "payload": {"to": "11", "amount": 5000.0}})
