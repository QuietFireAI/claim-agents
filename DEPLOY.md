# DEPLOY - Claim Swarm (Insurance) (v0.2, ratified 2026-07-11; extended & ratified 2026-07-18 - owner sign-off)
...
- FNOL channels (phone/web/portal) (IN)
- Policy admin system (IN)
- Estimating platform (OUT+IN)
- Payments/reserves core (OUT)
- Inspection/adjuster vendors (OUT+IN)
- Document intake (email/upload) (IN)
...
closed at load.
- `config/carrier_rule_table.json` - Coverage-adjacent routing rules and deadlines per line of business
- `config/reserve_rules.json` - Reserve-setting rules the records lane may cite
- `config/fraud_signal_taxonomy.json` - Ratified signal categories 09 reports as facts
- `config/vendor_panel.json` - Adjuster/inspection/mitigation panel with credentials
- `config/message_templates.json` - Approved claimant communication templates
- `config/authority_signers.json` - Humans authorized on payment.authority
